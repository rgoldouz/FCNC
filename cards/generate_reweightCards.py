...
Ivalue        = [3    ,2     ,10    ,2    ,10    ,1    ,0.5  ,10   ,0.07  ,10  ,1.5    ,1.5  ,1.5  ,1.5  ,1.5     ,0.4    ]
   ...
   for WC1 in couplingsName:
       ...
       for WC2 in couplingsName:
           if WC1 == WC2:
               ...
               for wcIndex in couplings[couplingsName.index(WC2)]:
                   rwgtCards = rwgtCards  + '    set param_card ' + wcIndex + ' 0.0001'  + '\n'
open('tllFCNC_reweight_card.dat', 'wt').write(rwgtCards)
os.system('mkdir tllFCNC')
os.system('cp tllFCNC_proc_card.dat tllFCNC')
os.system('cp tllFCNC_reweight_card.dat tllFCNC')
os.system('cp tllFCNC_customizecards.dat tllFCNC')
os.system('cp tllFCNC_run_card.dat tllFCNC')

for WC1 in couplingsName:
    os.system('rm -rf tllFCNC'+WC1)
    os.system('mkdir tllFCNC'+WC1)
    os.system('cp tllFCNC_extramodels.dat tllFCNC'+WC1 + '/tllFCNC'+WC1 +'_extramodels.dat')
    os.system('cp tllFCNC_run_card.dat tllFCNC'+WC1 + '/tllFCNC'+WC1 +'_run_card.dat')
...
#    Ccards = Ccards + 'set dynamical_scale_choice 3\n'
    ...
    open('tllFCNC'+WC1+'_customizecards.dat', 'wt').write(Ccards)
    os.system('mv tllFCNC'+WC1+'_customizecards.dat tllFCNC'+WC1)
    process = ''
#    if WC1 in C4F:
...
#        process = process + 'import model dim6top_LO_UFO-full --modelname' + '\n'
    ...
    process = process + 'add process p p > t~  l+ l- DIM6=0 FCNC=1 @1' + '\n'
    process = process + 'output tllFCNC' + WC1 + ' -f -nojpeg'
    open('tllFCNC'+WC1 +'_proc_card.dat', 'wt').write(process)
    os.system('mv tllFCNC'+WC1+'_proc_card.dat tllFCNC'+WC1)
