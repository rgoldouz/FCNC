...
def f(name):
   ...
   #     print '-nevent:' + str(genEventCount) +' -neventsweightSumw:'+str(genEventSumw) +' -neventsweightSumw/w:'+str(genEventSumw/abs(evtTree.genWeight))
    ...
    print 'REZA' + name.split('/')[8]
    for key, value in MCSAMPLES.items():
        if key == name.split('/')[8]:
            print key + '  ' + str(len(nWeight)) + ' ' + str(nRuns)
...
#    MCSAMPLES = {}
    ...
    for key, value in dirSamples.items():
        ...
        for root, dirs, files in os.walk(value[1]):
            if len(files) > 0:
        #        print root
                if 'UL17_BNV_ST_TSUE_DAS' not in root:
                    continue
                if 'data' in root:
                    for key, value in MCSAMPLES.items():
                        if key in root:
    ...
    #                        print 'neventsweight:'+str(neventsweight)
#    print Slist 
    Parallel(n_jobs=20, require="sharedmem")(delayed(f)(i) for i in Slist)
...
#        print(key, ' : ', value)
    ...
    #
#    print text
    open('test_nano.py', 'wt').write(text)
