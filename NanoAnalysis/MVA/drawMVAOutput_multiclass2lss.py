...
TMVA.Tools.Instance()
MVAs=["TU","TC"]
variables=["lep1Pt","lep2Pt","llDr","llDphi","llM","llPt", "jet1Pt", "bJetPt", "nJets", "tH_topMass","tH_HMass","tH_WtopMass","tH_W1HMass","tH_W2HMass","tH_HPt","tH_HEta","tH_topPt","tH_topEta","tH_drWtopB","tH_drW1HW2H"]
FCNC2l2q=["S"]
for MVA in MVAs:
    # Open the TMVA output file
    file = TFile.Open('tmp_TMVAClassification_'+MVA+'_2lss/TMVAOutput_'+MVA+'_2lss.root')
    ...
    for n,c in enumerate(FCNC2l2q):
        ...
        hist1 = file.Get('dataset/Method_BDT/BDT/MVA_BDT_Train_'+c)
        hist1.SetLineColor(ROOT.kBlue+2)
        hist1.SetFillColorAlpha(ROOT.kBlue, 0.3)
        hist1.Scale(1/hist1.Integral())
        HH.append(hist1)
        HHname.append('Train_Sig('+c+')')
        hist1 = file.Get('dataset/Method_BDT/BDT/MVA_BDT_Train_B')
        hist1.SetLineColor(ROOT.kRed+2)
        hist1.SetFillColorAlpha(ROOT.kRed, 0.3)
        hist1.Scale(1/hist1.Integral())
        HH.append(hist1)
        HHname.append('Train_BG')
        hist1 = file.Get('dataset/Method_BDT/BDT/MVA_BDT_'+c)
        hist1.SetLineColor(ROOT.kBlue+2)
        hist1.SetLineWidth(2)
        hist1.Scale(1/hist1.Integral())
        HH.append(hist1)
        HHname.append('Test_Sig('+c+')')
        hist1 = file.Get('dataset/Method_BDT/BDT/MVA_BDT_B')
        hist1.SetLineColor(ROOT.kRed+2)
        hist1.SetLineWidth(2)
        hist1.Scale(1/hist1.Integral())
        HH.append(hist1)
        HHname.append('Test_BG')
        compareHists(HH,HHname, '2lss',MVA,c,'MVA output')
    ...
    for numvar, namevar in enumerate(variables):
        ...
        HH.append(hist1)    
        HHname.append('BG')
        hist1 = file.Get('dataset/InputVariables_Id/'+namevar+'__Signal_Id')
        hist1.Scale(1/hist1.Integral())
        hist1.SetLineColor(colors[n])
        hist1.SetLineWidth(2)
        HH.append(hist1)
        HHname.append(c)
        compareHists(HH,HHname, '2lss',MVA,'IV_'+namevar,namevar)
